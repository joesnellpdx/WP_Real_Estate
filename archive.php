			...
			<?php if ( have_posts() ) : ?>
				...
				<header class="page-header">
					<h1 class="page-title">
						<?php
							if ( is_category() ) {
								printf( __( 'Category Archives: %s', 'wp_real_estate' ), '<span>' . single_cat_title( '', false ) . '</span>' );

							} elseif ( is_tag() ) {
								printf( __( 'Tag Archives: %s', 'wp_real_estate' ), '<span>' . single_tag_title( '', false ) . '</span>' );
								...
								*/
								the_post();
								printf( __( 'Author Archives: %s', 'wp_real_estate' ), '<span class="vcard"><a class="url fn n" href="' . get_author_posts_url( get_the_author_meta( "ID" ) ) . '" title="' . esc_attr( get_the_author() ) . '" rel="me">' . get_the_author() . '</a></span>' );
